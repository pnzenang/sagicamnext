    ...
    <div className='grid gap-3'>
      {requests.length > 0 ? (
        <div className='flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between'>
          ...
          {normalizedSearch ? (
            <Button type='button' variant='outline' size='sm' className='h-10 w-full sm:w-fit' onClick={() => setSearch('')}>
              <X />
              Clear
            </Button>
          ) : null}
      ...
      {requests.length === 0 ? (
        ...
        <Card className='rounded-lg'>
          <CardContent className='py-8 text-center'>
            ...
            <p className='text-muted-foreground mt-1 text-sm'>Try another name, matriculation number, sponsor code, or status.</p>
          </CardContent>
        </Card>
      ) : (
